...
The \KalrayK core provides four 64-bit counters that allow to monitor various
performance events, with the ability to trigger interrupts on overflow.

\subsubsection{Performance Monitor Counters}

PM0, PM1, PM2 and PM3 registers contain the value of the counters of the performance
...
\subsubsection{Performance Monitor Count Registers (PM0, PM1, PM2 and PM3)}

PM0, PM1, PM2 and PM3 are the four 64-bit counters, they are reset to 0x0.

\subsubsection{Performance Monitor Control Register (PMC)}
...
The PMC register, which value out of reset is 0x0000408000, contains the following
...
This bit is automatically set by hardware when it is already 0 and a performance monitor whose PM$<i>$IE bit is set overflows
...
\end{itemize}
The four Performance Monitors will start counting again as soon as the software clears PM.SAV
...
This 2-bit field is automatically set by the HW when PM.SAV = 0 and a performance monitor whose PM$<i>$IE bit 
...
\hline
 23 & DCHE & DCache Hit Event & PC & +1 for each architecturally executed load/DTOUCHL/store/DZEROL that hits in the L1 data cache (a bundle might be responsible for +2 if a misaligned load/store spans over two cache lines)
...
\hline
 25 & DCME & DCache Miss Event & PC  & +1 for each architecturally executed load/DTOUCHL/store/DZEROL that misses in the L1 data cache (a bundle might be responsible for +2 if a misaligned load/store spans over two cache lines)
 Does not increment if the access is canceled (e.g. by a MMU trap) or uncached\\
\hline
\end{tabularx}
\caption{Performance counting conditions.}
...
\hline
43 & ESE & Executed Store Event & PC & +1 for each architecturally executed store or DZEROL (does not increment on conditional stores/DZEROL whose condition is not met).\\
\hline
44 & ESNBE & Executed Store N Bytes Event & PC & +n for each architecturally executed store/DZEROL, with n is the number of bytes stored (n is 64 for DZEROL).\\
\hline
