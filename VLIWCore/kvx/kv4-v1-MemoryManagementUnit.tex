...
a unified address space for instructions and data. It provides four primary
functions: \begin{itemize}

\item Perform address translation from 52-bit  virtual addresses, optionally
associated to a 9-bit Address Space Number (ASN), to 48-bit physical addresses.
...
\item Enforce memory sharing policies for each virtual memory page.
...
and 1GB. For each TLB entry, the (virtual and physical) addresses must be
...
the protection attributes authorize the memory store/DZEROL/atomic instruction execution, however the entry ES value is ``Present'' (CP = uncached).
...
\item[DMISALIGN] Data misaligned access to a device area (CP = 00) or data misaligned access spanning over two pages with different cache or sharing policies
...
locked TLB (LTLB) with 16 entries, and a 8-way set-associative joined TLB (JTLB)
...
\bf Page Size & 4KB & 64KB & 2MB & 1GB \\
\hline \end{tabular}
}

\item[PA2] Protection Attributes

The sixteen values encoded in PA2 are interpreted as different access protections
depending on the PS.MMUP Privilege Mode bit:

{\small
\begin{tabular}{|l|l|l|l|l|l|l|l|l|} \hline
\bf PA2 Value & 0 & 1 & 2 & 3 & 4 & 5 & 6 & 7 \\
\hline
\bf PS.MMUP = 0 & NA & NA & NA & NA & NA & R & R & R \\
\hline
\bf PS.MMUP = 1 & NA & R & RW & RX & RWX & R & RW & RX \\
\hline \hline
\bf PA2 Value & 8 & 9 & 10 & 11 & 12 & 13 & 14 & 15 \\
\hline
...
\item[CP2] Cache Policy

The values encoded in CP2 (= Cache Policy) are interpreted as follows:

{\small
\begin{tabular}{|l|l|l|l|l|} \hline
\bf CP2    & \bf Type & \bf L2 policy (I + D)   & \bf L1D policy     & \bf L1I policy    \\ \hline
\bf 0000   & Memory   & L2 uncached             & L1D uncached       & L1I uncached      \\ \hline
\bf 0001   & Memory   & L2 uncached             & L1D uncached       & L1I cached        \\ \hline
\bf 0010   & Memory   & L2 uncached             & L1D cached         & L1I uncached      \\ \hline
\bf 0011   & Memory   & L2 uncached             & L1D cached         & L1I cached        \\ \hline
\bf 0100   & Memory   & L2 cached               & L1D uncached       & L1I uncached      \\ \hline
\bf 0101   & Memory   & L2 cached               & L1D uncached       & L1I cached        \\ \hline
\bf 0110   & Memory   & L2 cached               & L1D cached         & L1I uncached      \\ \hline
\bf 0111   & Memory   & L2 cached               & L1D cached         & L1I cached        \\ \hline
\bf 1000   & Device   & L2 uncached             & L1D uncached       & L1I uncached      \\ \hline
\bf other  & Reserved & unpredictable           & unpredictable      & unpredictable     \\ \hline \end{tabular}
}
...
\item[SP] Shareability Policy

The values encoded in SP (= Shareability Policy) are interpreted as follows:

{\small
\begin{tabular}{|l|l|} \hline
\bf SP   & \bf Policy       \\ \hline
\bf 00   & Not shared       \\ \hline
\bf 01   & Cluster shared   \\ \hline
\bf 10   & Chip shared      \\ \hline
\bf 11   & Reserved         \\ \hline \end{tabular}
}

These values drive coherency bits on the SMEM protocol, which define how the SMEM system interacts with the top-level
interconnect when accessing data in the page.
...
Atomic instructions (ALCLRW, ALCLRD, ALADDW, ALADDD, ACSWAPW, ACSWAPD) are considered as read/write accesses. As a result, they need both ``R'' and ``W'' rights in the PA2 field to execute without triggering a PROTECTION trap.
...
The memory space is globally considered as unprotected (as if PA2 = 13: {RWX, RWX}). All addresses are considered as in Device space (CP2 = 8, implying no misalignment is allowed)
except pre-defined regions that are treated as fully cached (CP2 = 7), corresponding to memories (SMEM, DDR, boot ROM,...). Refer to the chip top-level memory map for the definition
of these regions.

Furthermore, fetch or data accesses to virtual addresses above the range allowed by the physical address width (above 0xFFFF\_FFFF\_FFFF)
will respectively trigger PSYSERROR and DSYSERROR traps (as there is no corresponding flat-mapped physical address in the current implementation of the physical memory map).

\subsection{Local Cluster address space}
The local cluster address space is aliased : it can be seen from 2 address ranges, the "local" one [0MB; 16MB[ and the "absolute" one [(cluster\_id+1)*16MB; (cluster\_id+2)*16MB[.
To ensure the L1-coherency and prevent the same memory data to be cached twice with 2 aliased addresses, the MMU always (even when off) translates the absolute address space of the current cluster to the local one, whatever the FN of MMU pages.
